...
2. **Download SQL File**: Obtain the [little_lemon.sql](./little_lemon.sql) file from this repository.

3. **Import and Execute in MySQL Workbench**:
   - Open MySQL Workbench.
   - Navigate to `Server` > `Data Import`.
   - Choose `Import from Self-Contained File` and load the `little_lemon.sql` file.
...
```sql
CREATE PROCEDURE `little_lemon`.`CheckBooking`(IN booking_date DATE, IN table_number INT)
BEGIN
...
```sql
CREATE PROCEDURE `little_lemon`.`UpdateBooking`(
...
```sql
CALL `little_lemon`.`UpdateBooking`(9, '2022-11-15');
```
...
```sql
CREATE PROCEDURE `little_lemon`.`AddBooking`(
...
```sql
CALL `little_lemon`.`AddBooking`(17, 1, '2022-10-10', 5, 2);
```
...
```sql
CREATE PROCEDURE `little_lemon`.`CancelBooking`(IN booking_id_to_cancel INT)
BEGIN
...
```sql
CALL `little_lemon`.`CancelBooking`(9);
```
...
```sql
CREATE PROCEDURE `little_lemon`.`AddValidBooking`(IN new_booking_date DATE, IN new_table_number INT, IN new_customer_id INT, IN new_staff_id INT)
BEGIN
    ...
    ELSE
        INSERT INTO `little_lemon`.`Bookings`(`Date`, `TableNumber`, `CustomerID`, `StaffID`)
...
BEGIN
  ...
  SELECT COUNT(*) INTO orderExistence FROM `little_lemon`.`Orders` WHERE OrderID = orderIDToDelete;

  IF orderExistence > 0 THEN
    DELETE FROM `little_lemon`.`OrderDeliveryStatuses` WHERE OrderID = orderIDToDelete;

    DELETE FROM `little_lemon`.`Orders` WHERE OrderID = orderIDToDelete;
